...
public:
  ...
  // open and close
  int Open(const char *path, unsigned int options = S_FILE_OP_OPEN_ALWAYS);
  void openStdout();
  void Close();
  ...
  // byte operation
  int readAt(const size_t offset, const size_t size, void *buffer,
             int *const bytes);

  // handle
  fileHandle getHandle() const { return _fileHandle; }
};
...
// set the file handle
void fileOp::setFileHandle(fileHandle handle) { _fileHandle = handle; }

// check the validation of the file handle
bool fileOp::isValid() { return (_fileHandle != S_INVALID_HANDLE_FD_VALUE); }

// open a file and get the file handle
int fileOp::Open(const char *path, unsigned int options) {
  int rc = S_OK;
  int mode = O_RDWR;
  ...
  } while ((_fileHandle == -1) && (errno == EINTR));
...
void fileOp::seekToEnd() { S_lseek(_fileHandle, 0, SEEK_END); }
...
int fileOp::readAt(const size_t offset, const size_t size, void *buffer,
                   int *const bytes) {
  int rc = S_OK;
  ssize_t bytesRead = 0;
